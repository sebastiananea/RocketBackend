...
mercadopago.configure({
    access_token: 'TEST-102980178963781-121022-2bd48d71c22e89069502432b4ad8d8d8-379616593'
});


//venta paso 1
router.post('/ask-pay', async (req, res) => {
    const { institution, email, id_orden } = req.body
...
router.get('/control-pay', async (req, res) => {
    ...
    try{
        await Sales.findOneAndUpdate(
            ...
            (err, result) => {
                ...
                if (result) return res.redirect('http://localhost:3000/payment/admin/verify-sale/true');
                if (err) return res.redirect('http://localhost:3000/payment/admin/verify-sale/true')
            }
        );
    ...
    return res.redirect(`http://localhost:3000/institucion/admin/verify-sale/true`);
})
